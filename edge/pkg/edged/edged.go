...
func (e *edged) syncPod(podCfg *config.PodConfig) {
	...
	for {
		...
		klog.Infof("syncPod resource: %s, source: %s", result.GetResource(), result.GetSource())
	...
	for _, list := range lists {
		...
		if filterPodByNodeName(&pod, e.nodeName) {
			...
			if pod.DeletionTimestamp == nil {
				klog.Infof("handlePodListFromMetaManager, Pod %s is added to pod list", pod.Name)
				pods = append(pods, &pod)
			} else {
				klog.Infof("handlePodListFromMetaManagerPod %s is added to podDeletionQueue", pod.Name)
